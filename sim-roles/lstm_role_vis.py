...
def plot_trust2z(model):
    ...
    Z1 = np.zeros((45 - TRAJ_LEN, len(data)))
    Z2 = np.zeros((45 - TRAJ_LEN, len(data)))
    for idx, traj in enumerate(data):
        traj = torch.Tensor(traj)
        for snip in range(45 - TRAJ_LEN):
            x = traj[snip:snip+TRAJ_LEN,2:]
            z_pred = model.encode(x)
            Z1[snip, idx] = z_pred[0].item()
            Z2[snip, idx] = z_pred[1].item()
...
DATASET = "models/dataset.pkl"
TRAJ_LEN = 10

def main():

    modelname = 'models/lstm.pt'
