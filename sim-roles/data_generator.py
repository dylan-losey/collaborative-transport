...
class Player(pygame.sprite.Sprite):
    ...
    def update(self, delta_t):
        ...
        # choose human force
        f2 = fgoal * (1 - self.w1) + f1 * self.w1
        if self.count % 20 == 0:
            self.noise = np.random.normal(self.mu, self.w2, 2)
            # if np.random.random() < self.w3:
            #     self.bias = -1.0 * fobs
            # else:
            #     self.bias = 0.0 * fobs
        f2 += self.noise
    ...
    for count in range(num_of_runs):
        ...
        for w1 in W1:
            ...
            for w2 in W2:
                ...
                file_name = "simulated-data/r" + str(count) + "_w1_" + str(w1) + "_w2_" + str(w2) + ".pkl"
                ...
                while True:
                    ...
                    data.append([player.w1, player.w2, player.x, player.y] + list(player.goal) + list(player.obs) + player.f1 + player.f2)
